...
int main(int argc, char** argv)
{
	...
	if (std::cin.fail())
	{
		...
		x = -1;
		// Note2: cin is in an "error state" -- we handled it. Tell cin it's all good by
	...
	if (std::cin.fail())
	{
		std::cout << "Invalid float. Setting y to -1.0\n";
		y = -1.0;
		// Note2: cin is in an "error state" -- we handled it. Tell cin it's all good by
		// Note3: Make sure to do clear before ignore
		std::cin.clear();
	...
	//std::cout << "Enter a string for s (no whitespace): ";
	//std::cin >> s;
	//std::cin.ignore(100000, '\n');

	// If we want any type of text
	// Note4: Since we have some input already, there's likely a ewline still
	// in the buffer.
